...
update_smartdns() {
    local SMARTDNS_DIR="$BUILD_DIR/feeds/small8/smartdns"
    local LUCI_APP_SMARTDNS_DIR="$BUILD_DIR/feeds/small8/luci-app-smartdns"

    if [ -d "$SMARTDNS_DIR" ]; then
        echo "正在应用 smartdns 优化补丁..."
        install -Dm644 "$BASE_PATH/patches/100-smartdns-optimize.patch" "$SMARTDNS_DIR/patches/100-smartdns-optimize.patch"
    else
        echo "警告：未找到 smartdns 目录: $SMARTDNS_DIR"
    fi

    if [ -d "$LUCI_APP_SMARTDNS_DIR" ]; then
        echo "luci-app-smartdns 来自 small8 feed"
    else
        echo "警告：未找到 luci-app-smartdns 目录: $LUCI_APP_SMARTDNS_DIR"
    fi
}
